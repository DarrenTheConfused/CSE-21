public class Lab21_5 {
	...
	public static long squareSum(int max) {
		...
		for (int j = 1; j <= max; j++) {
			square += (j * j);
			System.out.println("squareSum " + j + " value " + square);
		}
		return square;
	}
	//square for array
	public static long square4Array(int max) {
		long square = 0;
		for (int j = 1; j <= max; j++) {
			square += (j * j);
		}
	...
	public static long squareSum(long[] arr, int max) {
		...
		for (int k = min; k <= max; k++) {
			arr[k] = square4Array(k);
			System.out.println("squareSum[" + k + "] value " + arr[k]);
		}
		return arr[max];
	}
	//fibonacci
	public static long fib4arr(int max) {
		long sum = 0;
		long zero = 0;
		long one = 1;
		long[] fibb = new long[max + 2];
		fibb[0] = zero;
		fibb[1] = one;
	...
	public static long fib(int max) {
		...
		System.out.println("Fibonacci 0 value 0");
		System.out.println("Fibonacci 1 value 1");
		
		for (int j = 2; j <= max; j++) {
			sum = zero + one;
			fibb[j] = sum;
			System.out.println("Fibonacci " + j + " value " + sum);
			zero = one;
			one = sum;		
		}
		return sum;
	...
	public static long factorial(int max) {
		...
		for (int i = 2; i <= max; i++) {
			...
			System.out.println("Factorial " + i + " value " + newNum);
		}
	...
	public static long helpFact(int max) {
		long help = 1;
		for (int i = 1; i < max; i++) {
			help = help * i;
		}
	...
	public static long fact4arr(int max) {
		long sum = helpFact(max);
		return sum;
	...
	public static long factorial(long[] arr, int max){
		...
		for (int k = min; k <= max; k++) {
			arr[k] = fact4arr(k);
			System.out.println("Factorial[" + k + "] value " + arr[k]);
		}
		return arr[max];
	}
